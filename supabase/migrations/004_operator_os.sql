-- ============================================================
-- Migration 004 — Operator OS (Complete / Run This One)
-- Safe to re-run — fully idempotent (IF NOT EXISTS / ADD COLUMN IF NOT EXISTS)
--
-- What this does:
--   A) Fix clients table — add 15 missing columns
--   B) Fix phase_fields.field_type constraint — add 'tasklist'
--   C) Drop duplicate phase trigger (JS in AddClientModal.jsx is source of truth)
--   D) Create 5 missing tables: contacts, tasks, subtasks, client_payments, expenses
--   E) Create Operator OS tables: companies, company_milestones, availability_rules,
--      day_rituals, weekly_plans, weekly_outcomes, daily_plans, daily_plan_tasks, daily_reviews
--   F) Add deferred FK constraints on tasks
--   G) Seed: MoveOn + Adexra companies, 8 MoveOn milestones, 3 default day rituals
-- ============================================================
...
CREATE TABLE IF NOT EXISTS tasks (
  ...
  -- FKs added in Section F after dependent tables exist
...
-- ============================================================
-- SECTION H — Duplicate phase cleanup check (run manually if needed)
--
-- Detect clients with > 4 phases (trigger + JS both fired):
--   SELECT client_id, count(*) FROM client_phases
--   GROUP BY client_id HAVING count(*) > 4;
--
-- If rows returned, delete the trigger-created set (order_index 1-4):
--   DELETE FROM client_phases
--   WHERE order_index BETWEEN 1 AND 4
--   AND client_id IN (
--     SELECT client_id FROM client_phases
--     GROUP BY client_id HAVING count(*) > 4
--   )
--   AND id NOT IN (
--     SELECT MIN(id) FROM client_phases
--     WHERE order_index BETWEEN 0 AND 3
--     GROUP BY client_id, phase_name
--   );
--   -- Review before running. The JS-created phases have order_index 0-3.
-- ============================================================
